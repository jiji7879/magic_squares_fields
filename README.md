...
| `pythagorean_fields_q1.py`     | Verifies the (Q\equiv1\pmod4) square case below the theoretical bound (Q<77).     |
...
| `q3_results.txt`               | Output from the (Q\equiv3\pmod4) square computation.                              |
...
When (Q\equiv1\pmod4), (-1) is a square in (\mathbb F_Q), so the center-zero construction is especially useful.

When (Q\equiv3\pmod4), (-1) is not a square in (\mathbb F_Q), so the center-one construction is used.

The paper proves that:

* every (Q\equiv1\pmod4) finite field with (Q\ge77) is non-Parker;
* every (Q\equiv3\pmod4) finite field with (Q\ge553736) is non-Parker;
* every odd finite field with (Q\ge1037) is non-(3)-Parker for cubes.
...
Run the (Q\equiv1\pmod4) square computation:

```bash
python pythagorean_fields_q1.py
```

Run the (Q\equiv3\pmod4) square computation:
...
The (Q\equiv3\pmod4) computation checks prime powers (Q<553736) satisfying

[
Q\equiv3\pmod4,
]

not merely prime fields (\mathbb F_p) with (p\equiv3\pmod4).
